...
An interactive research agent that autonomously searches the web and generates structured Markdown reports.
...
main.py (REPL)
    └── agent.py
            ...
            ├── Memory: MemorySaver (per-session history)
            └── ReAct loop (LangGraph create_react_agent)
                    ├── web_search()   — DuckDuckGo via ddgs
                    ├── read_url()     — full text via trafilatura (≤5000 chars)
                    └── write_report() — saves Markdown to output/
```
...
| `MODEL_NAME` | `claude-3-5-haiku-20241022` | LLM model |
| `MAX_SEARCH_RESULTS` | `5` | Results per web_search |
| `MAX_URL_CONTENT_LENGTH` | `5000` | Max chars from read_url |
